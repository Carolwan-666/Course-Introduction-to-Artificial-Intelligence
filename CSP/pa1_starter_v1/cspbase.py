...
class Variable: 
    ...
    def __init__(self, name, domain=[]):
        ...
        self.curdom = [True] * len(domain)      #using list 确定哪些域值是"当前的"
        #for bt_search
    ...
    def cur_domain(self):
        ...
        vals = []
        if self.is_assigned():  # assignmentValue != None
            vals.append(self.get_assigned_value())
        else:
            for i, val in enumerate(self.dom):
                if self.curdom[i]:  # self.curdom[i] == True:
                    vals.append(val)
        return vals
    ...
    def in_cur_domain(self, value):
        '''check if value is in CURRENT domain (without constructing list)
           ...
           domain'''
        # 如果是assigned, 只有 assigned value is viewed as being in current
        # domain
        if not value in self.dom: #如果value 不在domain中直接 return false
            return False
        if self.is_assigned():  # assignmentValue != None
    ...
    def assign(self, value):
        ...
        if self.is_assigned() or not self.in_cur_domain(value): #value 被assign了 或 value不在domain里面
    ...
    def value_index(self, value):
        ...
        return self.dom.index(value)  # dom 里的值会有重复的可能吗？
...
class Constraint: 
    ...
    def __init__(self, name, scope): 
        ...
        self.name = name
        self.sat_tuples = dict()  # tuple:T/F
    ...
    def has_support(self, var, val):
        ...
        '''
        if (var, val) in self.sup_tuples:  # (X, 1) in sup_tuple
            for t in self.sup_tuples[(var, val)]:  # for tp in sup_tuples[(var, val)] -->(1, 1, 2)
